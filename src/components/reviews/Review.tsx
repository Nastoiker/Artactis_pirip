...
export const Review = ({ name, comment, img }: ReviewProps) => {
  return (
    ...
    >
      <img
        src={img}
        className="mx-auto border-teal-500 border-t-8 w-72 h-72 rounded-full"
        alt="Review"
      />
      <div className={"ml-0 space-y-3   text-start"}>
        <h1 className={"ml-0 text-4xl text-teal-500"}>{name}</h1>
        <p className={" text-white  text-xl leading-8 break-words w-[80vw] sm:max-w-[800px]"}>{comment}</p>
      </div>
    </div>
  );
};
